...
@csrf_exempt
def autocomplete_object_execute(request):
    instance = Filter(app_label=request.GET.get('app_label'),
                      model_name=request.GET.get('model')
                      ).get_instance(int(request.GET.get('id')))
    try:
        result = getattr(instance, request.GET.get('method'))(request)
    except:
        try:
            result = getattr(instance, request.GET.get('method'))()
        except:
            result = str(getattr(instance, request.GET.get('method')))
    return HttpResponse(json.dumps(result, cls=Codec), content_type='application/json')
